...
def erg_booking(request):
    if request.method == "POST":
        ...
        otherBookingsOnSameErg = ErgBooking.objects.filter(
            erg=booking.erg.id, date=booking.date
        )
        otherBookingsIMade = ErgBooking.objects.filter(
            person=request.user.id, date=booking.date
        )
        ...
        for otherBooking in otherBookingsOnSameErg | otherBookingsIMade:
            ...
            book1End = time_plus(
                booking.startTime, datetime.timedelta(hours=booking.hours)
            )

            book2Start = otherBooking.startTime
            book2End = time_plus(
                otherBooking.startTime, datetime.timedelta(hours=otherBooking.hours)
            )
            if (
                (book1Start >= book2Start and book1End < book2End)
                or (book1Start <= book2Start and book1End >= book2Start)
                or (book1Start < book2End and book1End > book2End)
            ):
            ...
            "ergBookings": [
                {
                    ...
                    "bookings": ErgBooking.objects.filter(erg=erg.id, date=d),
                }
                for erg in Erg.objects.all()
            ],
        }
        for d in dates
    ]
...
def erg_manager_teams(request, team_id):
    ...
    workoutProfiles = [
        {
            "user": User.objects.get(id=it.person.id),
            "workoutDatas": [
                workout_create_data(w)
                for w in ErgWorkout.objects.filter(person=it.person.id)
            ],
        }
        for it in InTeam.objects.filter(team=team_id)
    ]
...
def signup_user(request):
    ...
    if request.method == "POST":
        ...
        user = User.objects.create_user(
            username=crsid.lower(), email=crsid + "@cam.ac.uk", password=password
        )
        user.first_name = first_name
        user.last_name = last_name
        user.save()

        teamNovice = InTeam.objects.create(
            person_id=user.id, team_id=Team.objects.get(name="Novices General").id
        )
        teamNovice.save()
        teamGeneral = InTeam.objects.create(
            person_id=user.id, team_id=Team.objects.get(name="General").id
        )
        teamGeneral.save()
        if team == "men":
            teamGeneral = InTeam.objects.create(
                person_id=user.id, team_id=Team.objects.get(name="Mens General").id
            )
            teamNovice2 = InTeam.objects.create(
                person_id=user.id, team_id=Team.objects.get(name="Mens Novices").id
            )
            teamGeneral.save()
            teamNovice2.save()
        ...
        if team == "women":
            teamGeneral = InTeam.objects.create(
                person_id=user.id, team_id=Team.objects.get(name="Womens General").id
            )
            teamNovice2 = InTeam.objects.create(
                person_id=user.id, team_id=Team.objects.get(name="Womens Novices").id
            )
            teamGeneral.save()
            teamNovice2.save()
...
def signup_users_bulk(csv):
    ...
    for user in users:
        try:
            ...
            teamNovice = InTeam.objects.create(
                person_id=user["object"].id,
                team_id=Team.objects.get(name="Novices General").id,
            )
            teamNovice.save()
            teamGeneral = InTeam.objects.create(
                person_id=user["object"].id, team_id=Team.objects.get(name="General").id
            )
            teamGeneral.save()
            if user["team"] == "men":
                teamGeneral = InTeam.objects.create(
...
def outing_manager_overview(request):
    ...
    od = [
        {
            "outing": outing,
            "rowerCount": InOuting.objects.filter(outing=outing.id)
            .filter(type="RW")
            .count(),
            "coxCount": InOuting.objects.filter(outing=outing.id)
            .filter(type="CX")
            .count(),
            "coachCount": InOuting.objects.filter(outing=outing.id)
            .filter(type="CC")
            .count(),
            "rowerSignup": Available.objects.filter(outing=outing.id)
            .filter(type="RW")
            .count(),
            "coxSignup": Available.objects.filter(outing=outing.id)
            .filter(type="CX")
            .count(),
            "coachSignup": Available.objects.filter(outing=outing.id)
            .filter(type="CC")
            .count(),
        }
        for outing in Outing.objects.filter(date__gte=today)
    ]

    context = {
        "outingData": sorted(
            od, key=lambda x: (x["outing"].date, x["outing"].meetingTime)
        )
    }
...
def signoff_outing(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            if (
                InOuting.objects.filter(
                    person=request.user.id, outing=form.cleaned_data["outing"]
                ).count()
                == 0
            ):
                Available.objects.filter(person=request.user.id).filter(
                    outing=form.cleaned_data["outing"], type=form.cleaned_data["type"]
                ).delete()
                return HttpResponse("Success!")
            else:
                return HttpResponse(
                    "You're currently set to be in this outing. Please get in contact with your captain if you can't make it."
                )
        else:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ErgBooking.objects.filter(person=request.user.id).filter(
                id=int(dict(request.POST.lists())["b_id"][0])
            ).delete()
            return HttpResponse("Success!")
        else:
            return HttpResponse("Error!")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ErgWorkout.objects.filter(person=request.user.id).filter(
                id=int(dict(request.POST.lists())["w_id"][0])
            ).delete()
            return HttpResponse("Success!")
        else:
            return HttpResponse("Error!")
...
def signup_outing(request):
    if request.method == "POST":
        ...
        if (
            Available.objects.filter(person=request.user.id)
            .filter(outing=a.outing, type=a.type)
            .count()
            > 0
        ):
        ...
        if a.type == "RW" and (
            InTeam.objects.filter(person=request.user.id, team=a.outing.team.id).count()
            == 0
        ):
...
def welcome(request):
    announcements = sorted(
        [
            ...
            if (
                (not x.isTeamSpecific)
                or InTeam.objects.filter(person=request.user.id)
                .filter(team=x.team.id)
                .count()
                > 0
            )
        ],
...
def my_outings(request):
    ...
    context = {
        "outings": sorted(
            filter(lambda x: x.date >= today, outings),
            key=lambda x: (x.date, x.meetingTime),
        ),
    }
...
def workout_create_data(w):
    return {
        "workout": w,
        "splitMin": math.floor(split(w) / 60),
        "splitSec": math.floor(split(w) % 60),
...
def outing_manager(request, outing_id):
    ...
    if request.method == "POST":
        ...
        for key, value in post_data.items():
            if key.startswith("row_"):
                ...
                if key == "cox":
                    io = InOuting(
                        person=User.objects.get(id=int(value[0])), type="CX", outing=o
                    )
                    io.save()
                elif key == "coach":
                    io = InOuting(
                        person=User.objects.get(id=int(value[0])), type="CC", outing=o
                    )
                    io.save()
    o = Outing.objects.get(id=outing_id)

    coxes = [
        x.person.id for x in InOuting.objects.filter(outing=outing_id).filter(type="CX")
    ]
    coaches = [
        x.person.id for x in InOuting.objects.filter(outing=outing_id).filter(type="CC")
    ]
    ...
    context = {
        ...
        "rowers": [
            x.person.id
            for x in InOuting.objects.filter(outing=outing_id).filter(type="RW")
        ],
        "coxes": coxes,
        "coaches": coaches,
    }
...
def view_past_outings(request, crsid):
    ...
    if request.method == "GET":
        context = {
            "in_outings": sorted(
                InOuting.objects.filter(person=user), key=lambda x: x.outing.date
            ),
            "user": user,
        }
...
def signup_page(request):
    ...
    if request.method == "GET":
        context = {
            "outings": sorted(
                Outing.objects.filter(date__gte=today),
                key=lambda x: (x.date, x.meetingTime),
            ),
            "teams": [
                {
                    "isMember": InTeam.objects.filter(
                        team=team.id, person=request.user.id
                    ).count()
                    > 0,
                    "value": team,
                }
            ...
            "availability_ids": [
                x.outing.id for x in Available.objects.filter(person=request.user.id)
            ],
            "availability_rw_ids": [
                x.outing.id
                for x in Available.objects.filter(person=request.user.id, type="RW")
            ],
            "availability_cx_ids": [
                x.outing.id
                for x in Available.objects.filter(person=request.user.id, type="CX")
            ],
            "availability_cc_ids": [
                x.outing.id
                for x in Available.objects.filter(person=request.user.id, type="CC")
            ],
            "comments": {
                x.outing.id: x.comment
                for x in Available.objects.filter(person=request.user.id)
            },
        }
...
def outing_analyser(request, s):
    ...
    for o in Outing.objects.all():
        ...
        outings_aug += [
            {"count": i, "outing": o, "people_string": ",".join(outing_names)}
        ]
